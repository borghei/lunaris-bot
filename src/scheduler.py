...
async def send_daily_reminder(app: Application):
    ...
    for user in db.get_all_active_users():
        ...
        if phase in ("pms", "menstruation"):
            ...
            should_send = True
        elif phase == "luteal" and cycle_day == 20:
            should_send = True
        ...
        try:
            tip = await generate_reminder(phase, cycle_day, recent_logs)

            if phase == "luteal" and cycle_day == 20:
                header = "⚡ Heads up darling: PMS starts in 2 days! Brace yourself 💜"
            else:
                header = f"Good morning darling! 🌸\n{info['label']} — Day {cycle_day}"
